...
import { Play, Clock, Pause, Music, History as HistoryIcon } from 'lucide-react'
import { Button } from './ui/button'
import { getHistory, Track } from '@/lib/music-api'
...
export default function History({ guildId, userId, channelId }: HistoryProps) {
    ...
    const fetchHistory = async () => {
        try {
            const data = await getHistory(guildId, userId)
            setTracks(data.recent || [])
        } catch (error) {
    ...
    const handlePlay = async (track?: Track) => {
        try {
            const query = track ? track.url : 'History'
            await play(query)
        } catch (error) {
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-end">
                <motion.div
                    ...
                    className="w-52 h-52 rounded-2xl bg-gradient-to-br from-orange-500 via-red-500 to-pink-500 flex items-center justify-center shadow-2xl shadow-orange-500/20"
                >
                    <HistoryIcon className="w-24 h-24 text-white" />
                </motion.div>
                ...
                <div className="flex-1 space-y-4 mb-2">
                    ...
                    >
                        ...
                        <h1 className="text-5xl md:text-7xl font-black tracking-tight text-white mt-2 mb-4">Recently Played</h1>
            ...
            <div className="bg-black/20 rounded-xl overflow-hidden backdrop-blur-sm border border-white/5">
                <div className="grid grid-cols-[auto_1fr_auto_auto] gap-4 px-4 py-3 border-b border-white/5 text-sm font-medium text-zinc-400 uppercase tracking-wider">
                    ...
                    <div>Title</div>
                    <div className="hidden md:block">Played At</div>
